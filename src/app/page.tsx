...
export default function CRM() {
  ...
  const [selChat,setSelChat] = useState(waChats[0])
  const [msg,setMsg] = useState('')
  const [chats,setChats] = useState(waChats)
  const allItems = NAV.flatMap(g=>g.items)
  const curLabel = allItems.find(i=>i.id===nav)?.label||'Dashboard'

  const sendWA = () => {
    if(!msg.trim()) return
    const m = {from:'me',text:msg,time:'Ahora'}
    setChats(p=>p.map(c=>c.id===selChat.id?{...c,last:msg,msgs:[...c.msgs,m]}:c))
    setSelChat(p=>({...p,msgs:[...p.msgs,m]}))
    setMsg('')
  }
    ...
    <div style={{display:'flex',height:'100vh',background:T.bg,fontFamily:"'Outfit',sans-serif",color:T.text,overflow:'hidden'}}>
      ...
      <aside style={{width:220,background:T.s1,borderRight:'1px solid '+T.border,display:'flex',flexDirection:'column',flexShrink:0,overflowY:'auto'}}>
        <div style={{padding:'18px 14px 16px',borderBottom:'1px solid '+T.border,display:'flex',alignItems:'center',gap:10}}>
          <div style={{width:32,height:32,borderRadius:9,background:'linear-gradient(135deg,'+T.accent+','+T.cyan+')',display:'flex',alignItems:'center',justifyContent:'center',fontSize:16}}>⬡</div>
        ...
        <div style={{padding:'12px 14px',borderTop:'1px solid '+T.border,display:'flex',alignItems:'center',gap:9}}>
          <div style={{width:28,height:28,borderRadius:'50%',background:'linear-gradient(135deg,'+T.accent+','+T.cyan+')',display:'flex',alignItems:'center',justifyContent:'center',fontSize:10,fontWeight:700,color:'#fff'}}>JA</div>
        ...
        <header style={{background:T.s1,borderBottom:'1px solid '+T.border,padding:'11px 22px',display:'flex',alignItems:'center',gap:14,flexShrink:0}}>
          ...
          <div style={{flex:1,display:'flex',alignItems:'center',gap:10,justifyContent:'flex-end'}}>
            <div style={{background:T.s3,border:'1px solid '+T.border,borderRadius:9,padding:'7px 12px',display:'flex',alignItems:'center',gap:7}}>
              ...
              <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Buscar..." style={{background:'none',border:'none',color:T.text,fontSize:12,width:180}}/>
            </div>
            <button style={{border:'none',borderRadius:9,padding:'7px 16px',background:'linear-gradient(135deg,'+T.accent+','+T.cyan+')',color:'#fff',fontSize:12,fontWeight:600,cursor:'pointer'}}>+ Nuevo</button>
          </div>
        </header>
  ...
  return <div style={{display:'flex',flexDirection:'column',gap:18}}>
    ...
    <div style={{display:'grid',gridTemplateColumns:'2fr 1fr 1fr',gap:16}}>
      <Card style={{padding:20}}>
        ...
        <div style={{display:'flex',alignItems:'flex-end',gap:7,height:110}}>
          {bars.map((h,i)=><div key={i} style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',gap:4}}>
            <div style={{width:'100%',borderRadius:'4px 4px 0 0',height:h+'%',background:i===3?'linear-gradient(180deg,'+T.accent+','+T.cyan+')':T.accent+'28'}}/>
      ...
      <Card style={{padding:18}}>
        ...
        {[{l:'Meta Ads',p:38,c:T.meta},{l:'Google Ads',p:27,c:T.google},{l:'WhatsApp',p:18,c:T.wa},{l:'Orgánico',p:12,c:T.green},{l:'Email',p:5,c:T.purple}].map(s=>(
      ...
      <Card style={{padding:18}}>
        ...
        {[{t:'Lead Meta — Marta Gil',d:'5 min',i:'🎯'},{t:'Carlos Rueda abrió propuesta',d:'22 min',i:'✉️'},{t:'Marcos Vega factura pagada',d:'1h',i:'💰'},{t:'Conversión Google Ads',d:'2h',i:'🔍'},{t:'WhatsApp — Ana López',d:'3h',i:'💬'}].map((a,i)=>(
    ...
    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:14}}>
      {[{name:'Meta Ads',icon:'f',bg:T.meta,stats:[{l:'Inversión',v:'€4,060'},{l:'Leads',v:'419'},{l:'CPL',v:'€9.69'},{l:'ROAS',v:'3.4x'}]},{name:'Google Ads',icon:'G',bg:T.google,stats:[{l:'Inversión',v:'€3,860'},{l:'Conv.',v:'180'},{l:'CPA',v:'€21.44'},{l:'CTR',v:'5.1%'}]},{name:'WhatsApp',icon:'W',bg:T.wa,stats:[{l:'Chats',v:'28'},{l:'Mensajes',v:'142'},{l:'Resp.',v:'94%'},{l:'T.resp.',v:'8min'}]}].map(intg=>(
        <Card key={intg.name} style={{padding:18}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:8}}>
            {intg.stats.map(s=><div key={s.l} style={{textAlign:'center',padding:'8px 4px',background:T.s3,borderRadius:8}}><div style={{fontWeight:800,fontSize:14}}>{s.v}</div><div style={{fontSize:9,color:T.muted,marginTop:1}}>{s.l}</div></div>)}
          </div>
        </Card>
      ))}
    </div>
...
function Contacts({search,sel,setSel}:{search:string,sel:any,setSel:any}) {
  ...
  return <div style={{display:'grid',gridTemplateColumns:sel?'1fr 300px':'1fr',gap:16}}>
    ...
    {sel&&<Card style={{padding:18,height:'fit-content',position:'sticky',top:0}}>
      ...
      {[{l:'Email',v:sel.email,i:'✉️'},{l:'Teléfono',v:sel.phone,i:'📞'},{l:'Ciudad',v:sel.city,i:'📍'},{l:'Valor',v:sel.value,i:'💰'},{l:'Origen',v:sel.source,i:'🎯'},{l:'Etapa',v:sel.stage,i:'⇢'}].map(f=><div key={f.l} style={{display:'flex',gap:9,padding:'8px 0',borderBottom:'1px solid '+T.border}}><span style={{fontSize:12}}>{f.i}</span><div><div style={{fontSize:9,color:T.muted,textTransform:'uppercase',letterSpacing:'0.6px'}}>{f.l}</div><div style={{fontSize:11,fontWeight:500,marginTop:1}}>{f.v}</div></div></div>)}
      <div style={{display:'flex',gap:7,marginTop:14}}>
        ...
        <button style={{flex:1,padding:'7px',borderRadius:8,border:'1px solid '+T.wa,background:T.wa+'15',color:T.wa,fontSize:10,fontWeight:600,cursor:'pointer'}}>WhatsApp</button>
        <button style={{flex:1,padding:'7px',borderRadius:8,border:'none',background:'linear-gradient(135deg,'+T.accent+','+T.cyan+')',color:'#fff',fontSize:10,fontWeight:600,cursor:'pointer'}}>✉️ Email</button>
      </div>
    </Card>}
  </div>
}
...
function Pipeline() {
  return <div>
    <div style={{display:'flex',gap:14,marginBottom:16,alignItems:'center'}}>
      ...
      <button style={{marginLeft:'auto',border:'none',borderRadius:9,padding:'7px 16px',background:'linear-gradient(135deg,'+T.accent+','+T.cyan+')',color:'#fff',fontSize:12,fontWeight:600,cursor:'pointer'}}>+ Oportunidad</button>
    </div>
    <div style={{display:'flex',gap:10,overflowX:'auto',paddingBottom:8}}>
      {kanban.map(stage=>{
        const total=stage.deals.reduce((a,d)=>a+parseInt(d.val.replace(/[^0-9]/g,'')),0)
        return <div key={stage.id} style={{minWidth:200,flexShrink:0}}>
          ...
          {stage.deals.map((d,i)=><div key={i} style={{background:T.s2,border:'1px solid '+T.border,borderLeft:'3px solid '+stage.color,borderRadius:9,padding:11,cursor:'grab',marginBottom:7}}>
  ...
  return <div style={{display:'flex',height:'100%'}}>
    <div style={{width:250,borderRight:'1px solid '+T.border,display:'flex',flexDirection:'column'}}>
      <div style={{padding:'14px 12px',borderBottom:'1px solid '+T.border}}>
        <div style={{display:'flex',alignItems:'center',gap:7,marginBottom:10}}><div style={{fontWeight:800,fontSize:14}}>WhatsApp Inbox</div><Pill c={T.green} sm>● Conectado</Pill></div>
      </div>
      ...
      {sel?<>
        ...
        <div style={{flex:1,overflowY:'auto',padding:'16px 18px',display:'flex',flexDirection:'column',gap:9}}>
          {sel.msgs.map((m:any,i:number)=><div key={i} style={{display:'flex',justifyContent:m.from==='me'?'flex-end':'flex-start'}}>
            <div style={{maxWidth:'70%',padding:'8px 12px',borderRadius:m.from==='me'?'12px 12px 4px 12px':'12px 12px 12px 4px',background:m.from==='me'?'linear-gradient(135deg,'+T.accent+','+T.cyan+')':T.s3,color:'#fff',fontSize:12,lineHeight:1.5}}>
        ...
        <div style={{padding:'10px 16px',borderTop:'1px solid '+T.border,display:'flex',gap:9,alignItems:'center'}}>
          ...
          <button onClick={send} style={{width:36,height:36,borderRadius:'50%',border:'none',background:'linear-gradient(135deg,'+T.wa+',#1aab53)',color:'#fff',fontSize:15,cursor:'pointer'}}>➤</button>
        </div>
...
function MetaAds() {
  ...
  return <div style={{display:'flex',flexDirection:'column',gap:16}}>
    <div style={{display:'flex',alignItems:'center',gap:12}}>
      <div style={{width:38,height:38,borderRadius:11,background:T.meta,display:'flex',alignItems:'center',justifyContent:'center',fontSize:18,color:'#fff',fontWeight:900}}>f</div>
    ...
    <div style={{display:'grid',gridTemplateColumns:'repeat(6,1fr)',gap:11}}>
      {[{l:'Inversión',v:'€4,060'},{l:'Impresiones',v:'794k'},{l:'Clics',v:'11,151'},{l:'Leads',v:'419',h:true},{l:'CPL',v:'€9.69'},{l:'ROAS',v:'3.4x',h:true}].map((k,i)=><div key={i} style={{padding:'11px 12px',background:T.s2,border:'1px solid '+T.border,borderRadius:11,textAlign:'center'}}><div style={{fontWeight:800,fontSize:17,color:k.h?T.green:T.text}}>{k.v}</div><div style={{fontSize:10,color:T.muted,marginTop:2}}>{k.l}</div></div>)}
    </div>
    <Card style={{overflow:'hidden'}}>
      <table style={{width:'100%',borderCollapse:'collapse'}}>
...
function GoogleAds() {
  ...
  return <div style={{display:'flex',flexDirection:'column',gap:16}}>
    <div style={{display:'flex',alignItems:'center',gap:12}}>
      <div style={{width:38,height:38,borderRadius:11,background:T.google,display:'flex',alignItems:'center',justifyContent:'center',fontSize:18,color:'#fff',fontWeight:900}}>G</div>
    ...
    <div style={{display:'grid',gridTemplateColumns:'repeat(6,1fr)',gap:11}}>
      {[{l:'Inversión',v:'€3,860'},{l:'Impresiones',v:'376k'},{l:'Clics',v:'5,230'},{l:'Conv.',v:'180',h:true},{l:'CPA',v:'€21.44'},{l:'CTR',v:'1.39%',h:true}].map((k,i)=><div key={i} style={{padding:'11px 12px',background:T.s2,border:'1px solid '+T.border,borderRadius:11,textAlign:'center'}}><div style={{fontWeight:800,fontSize:17,color:k.h?T.green:T.text}}>{k.v}</div><div style={{fontSize:10,color:T.muted,marginTop:2}}>{k.l}</div></div>)}
    </div>
    <Card style={{overflow:'hidden'}}>
      <table style={{width:'100%',borderCollapse:'collapse'}}>
        ...
        <tbody>{camps.map((c,i)=><tr key={i} style={{borderBottom:'1px solid '+T.border,cursor:'pointer'}}>
          ...
          <td style={{padding:'9px 11px'}}><Pill c={T.google} sm>{c.type}</Pill></td>
          <td style={{padding:'9px 11px'}}><Pill c={sC[c.status]||T.muted} sm>{c.status}</Pill></td>
          <td style={{padding:'9px 11px',fontSize:11,color:T.muted}}>{c.budget}</td>
          <td style={{padding:'9px 11px',fontSize:12,fontWeight:600}}>{c.spent}</td>
          <td style={{padding:'9px 11px',fontSize:11,color:T.muted}}>{c.clicks}</td>
    ...
    <div style={{display:'grid',gridTemplateColumns:'2fr 1fr',gap:16}}>
      <Card style={{padding:20}}>
        ...
        <div style={{display:'flex',alignItems:'flex-end',gap:7,height:120}}>
          {bars.map((h,i)=><div key={i} style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',gap:4}}><div style={{width:'100%',borderRadius:'4px 4px 0 0',height:h+'%',background:i===3?'linear-gradient(180deg,'+T.accent+','+T.cyan+')':T.accent+'28'}}/><div style={{fontSize:8,color:T.faint}}>{'EFMAMJJASOND'[i]}</div></div>)}
        </div>
      </Card>
      <Card style={{padding:20}}>
        <div style={{fontWeight:800,fontSize:14,marginBottom:16}}>ROI por Canal</div>
        {[{l:'Meta Ads',roi:'340%',c:T.meta},{l:'Google Ads',roi:'280%',c:T.google},{l:'Email',roi:'820%',c:T.accent},{l:'WhatsApp',roi:'650%',c:T.wa},{l:'Orgánico',roi:'∞',c:T.green}].map(c=><div key={c.l} style={{marginBottom:11}}>
